...
def extractor(args):
    ...
    feats_txt = torch.Tensor()
    image_ids = []
    for step in range(steps):
        print("Step: ", step)
        image, image_id, caption = next(dataiter)
        image = image.to(device)
        caption = caption.squeeze(1).to(device)
        feat_img, feat_txt = model(image, caption)
        feat_img = feat_txt.squeeze(-1).squeeze(-1).detach().cpu()
        feat_txt = feat_txt.squeeze(-1).squeeze(-1).detach().cpu()
        feats_img = torch.cat((feats_img, feat_img), 0)
        feats_txt = torch.cat((feats_txt, feat_txt), 0)
        image_ids.extend(image_id)
    save_file = os.path.join(args.save_path, f"{cfg.Qcate}_image_feats.pt")
    torch.save(feats_img, save_file)
    save_file = os.path.join(args.save_path, f"{cfg.Qcate}_txt_feats.pt")
    torch.save(feats_txt, save_file)
