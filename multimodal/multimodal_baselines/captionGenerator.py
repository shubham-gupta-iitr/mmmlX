...
def generate_beam(model, tokenizer, beam_size: int = 5, prompt=None, embed=None,
    ...
    with torch.no_grad():
        if embed is not None:
            generated = embed
            if tokens is None and prompt is not None:
                tokens = torch.tensor(tokenizer.encode(prompt))
                tokens = tokens.unsqueeze(0).to(device)
                generated2 = model.gpt.transformer.wte(tokens)
                # print(generated2.shape)
                generated = torch.cat((generated2,generated),dim=1)
        else:
            if tokens is None:
                tokens = torch.tensor(tokenizer.encode(prompt))
                tokens = tokens.unsqueeze(0).to(device)
...
def infer(image):
    ...
    if use_beam_search:
        ...
        prompts = ["which colors? ","what shapes and sizes? "]
        gen_captions = []
        gen_captions.append(generate2(model, tokenizer, embed=prefix_embed))
        for p in prompts:
            gen_captions.append(generate2(model, tokenizer, embed=prefix_embed,prompt=p).split('? ')[1])
        gen_captions = (' ').join(list(set(gen_captions)))
    return gen_captions
...
# p = Pool(3)
# p.map(infer, list of image paths)

dataset = json.load(open("WebQA_train_val.json", "r"))
image_paths=[]
for i,k in enumerate(dataset):
    if dataset[k]['split']=='val' and dataset[k]['Qcate']!='text':
...
lineidx_path = 'imgs.lineidx'
image_tsv = 'imgs.tsv'
with open(lineidx_path, "r") as fp_lineidx:
    lineidx = [int(i.strip()) for i in fp_lineidx.readlines()]

# image_id = image_paths[0]
img_cap = {}
for idx in range(len(image_paths[:])):
    image_id = image_paths[idx]
    with open(image_tsv, "r") as fp:
    ...
    img_cap[image_id] = infer(image)
    if idx % 1000 == 0:
        with open("img_cap_all.pkl","wb") as f:
            pickle.dump(img_cap,f)
with open("img_cap_all.pkl","wb") as f:
    # print(img_cap)
    pickle.dump(img_cap,f) 
