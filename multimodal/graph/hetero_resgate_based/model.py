...
from torch_geometric.nn import ResGatedGraphConv as SAGEConv
# from torch_geometric.nn import TAGConv as SAGEConv
from torch_geometric.nn.norm import BatchNorm, DiffGroupNorm
class ToyNet(torch.nn.Module):
    def __init__(self):
        super(ToyNet, self).__init__()

        # self.conv1 = SAGEConv(embed_dim, 2048)
        self.conv1 = SAGEConv(-1, 2048,aggr='max')
        #self.bn1 = BatchNorm(2048)
        # self.pool1 = TopKPooling(128, ratio=0.8)
        self.conv2 = SAGEConv(-1, 512,aggr='max')
        #self.bn2 = BatchNorm(1024)
        # self.pool2 = TopKPooling(128, ratio=0.8)
        self.conv3 = SAGEConv(-1, 128,aggr='max')
        ...
        # self._dgn_1 = DiffGroupNorm(-1, groups=2)
